...
function bmag_field_callback($option){
	...
	$value = get_option( BMAG_OPT );
	
	$optionname = BMAG_OPT.'[' .$option['name']. ']';
	$val = isset($value[$option['name']]) ? $value[$option['name']] : 'no opt';

	if($option['type'] == 'checkbox'){
		?><input type="checkbox" <?php checked($val || $val =='1'); ?> id="dd_<?php echo $option['name'];?>" name="<?php echo $optionname;?>">
			<label for="dd_<?php echo $option['name'];?>"><?php echo $option['description'] ?></label>
		 <?php
	}else{

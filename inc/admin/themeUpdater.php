...
class themeUpdater{
  ...
  public function get_old_params(){
    ...
    if(!$this->compare_version($this->version_new, $this->version_old)){
      $this->options = get_option(BMAG_OPT, false );
      ...
      update_option(BMAG_OPT, $this->options);
      return $this->options;
    }
        ...
        if($key == $this->old_meta_name) {
          ...
          if($old_meta != "") {
            foreach($this->rules_meta as $version => $changes){
              ...
              if(! update_post_meta( $current_post ->ID, BMAG_META, $old_meta)){
              add_post_meta( $current_post ->ID, BMAG_META, $old_meta);
            }
